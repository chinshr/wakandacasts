...
describe "Comments request" do
  ...
  it "creates when banned" do
    login FactoryGirl.create(:user, :banned_at => Time.now)
    visit episode_path(FactoryGirl.create(:episode), :view => "comments")
  ...
  it "updates a comment" do
    ...
    login user
    episode = FactoryGirl.create(:episode, :name => "Blast from the Past")
    comment = FactoryGirl.create(:comment, :content => "Hello world!", :episode_id => episode.id)
  ...
  it "destroys a comment" do
    ...
    episode = FactoryGirl.create(:episode, :name => "Blast from the Past")
    FactoryGirl.create(:comment, :content => "Hello world!", :episode_id => episode.id)
    visit episode_path(episode, :view => "comments")
    click_on "Delete"
  ...
  it "lists and search recent comments" do
    login FactoryGirl.create(:user, :admin => true)
    FactoryGirl.create(:comment, :content => "Hello world!")
    FactoryGirl.create(:comment, :content => "Back to the Future", :site_url => "http://example.com")
    visit comments_path
    page.should have_content("Hello world!")
