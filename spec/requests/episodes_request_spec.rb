...
describe "Episodes request" do
  it "lists published" do
    FactoryGirl.create(:episode, :name => "Blast from the Past", :published_at => 2.days.ago)
    FactoryGirl.create(:episode, :name => "Back to the Future", :published_at => 2.days.from_now)
    visit episodes_path
  ...
  it "has different views" do
    FactoryGirl.create(:episode, :name => "Blast from the Past", :description => "I am from 1960", :published_at => 2.days.ago)
    visit episodes_path
  ...
  it "searches by name" do
    FactoryGirl.create(:episode, :name => "Blast from the Past")
    FactoryGirl.create(:episode, :name => "Back to the Future")
    visit episodes_path
  ...
  it "filters by tag" do
    FactoryGirl.create(:episode, :name => "Blast from the Past", :tags => [FactoryGirl.build(:tag, :name => "Oldtimes")])
    FactoryGirl.create(:episode, :name => "Back to the Future")
    visit episodes_path
    click_on "Oldtimes"
    page.should have_content("Blast from the Past")
  ...
  it "contains show notes, comments, and similar episodes" do
    episode = FactoryGirl.create(:episode, :name => "Blast from the Past", :notes => "Show notes!", :position => 1)
    FactoryGirl.create(:comment, :content => "Hello world", :episode => episode)
    FactoryGirl.create(:episode, :name => "Star Wars", :position => 2)
    FactoryGirl.create(:episode, :name => "Past and Present", :position => 3)
    visit episodes_path
  ...
  it "redirects to episode when full permalink isn't used" do
    episode = FactoryGirl.create(:episode, :name => "Blast from the Past")
  ...
  it "creates a new episode with default position" do
    ...
    login FactoryGirl.create(:user, :admin => true)
    visit episodes_path
  ...
  it "edits an episode show notes as moderator" do
    FakeWeb.register_uri(:head, /.*/, :content_length => 123)
    login FactoryGirl.create(:user, :moderator => true)
